...
const logFormat = '{{timestamp}} {{title}} {{file}}:{{line}} ({{method}}) {{tid}} [{{user}}] {{message}}';
const sqlFormat = '{{timestamp}} {{title}} {{tid}} [{{user}}] {{message}}';
const mongoFormat = '{{timestamp}} {{title}} {{tid}} [{{user}}] {{message}}';
...
const logConfig = {
    ...
    preprocess: (data: any) => {
        ...
        const user = context.get('username');
        data.user = user ? user : '';
...
const logger = {
    ...
    mongo (...args: any[]) {
        if (exports.filter.console) console.log(...args);
        return exports.filter.mongo ? mongo.log(...args) : null;
    },
    log(...args: any[]) {
        if (exports.filter.console) console.log(...args);
